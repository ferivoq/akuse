...
const AniSkip = {
  ...
  getCurrentEvent: function (time: number, skipEvents: SkipEvent[]) {
    if (!skipEvents || skipEvents.length === 0) return;
  ...
  getEventName: function (skipEvent: SkipEvent) {
    for (const [name, value] of Object.entries(SkipEventTypes)) {
      if (value !== skipEvent.skipType) continue;
